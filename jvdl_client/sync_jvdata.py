...
# (エラー)になることを確認した。2026-07-03、JV-Data仕様書「蓄積系提供データ
# 一覧」「データ提供タイミング表」（公式SDK JVDTLABSDK4902 付属）でこの挙動が
# 仕様通りであることを裏付け済み: option=2 対応 dataspec は TOKU/RACE/TCOV/
# RCOV/SNAP 系のみで、SLOP/WOOD（調教データ）は対象外。
# weekly=True 指定時、このリスト外の dataspec は自動的に通常の差分取得
# (OPT_STORED_DIFF)にフォールバックする。
# TODO: TOKU（特別登録馬、日月夕刻配信）の取り込みは docs/review/ 配下の
# 調査記録を参照して別途検討（2026-07-03時点で未実装）。
...
def sync_from_jvlink(
    ...
    Args:
        ...
        weekly:       True なら JVOpen option=2 で今週分のみ取得する。木曜16:30頃に
                      枠番未確定の出走馬名表、金土12:00頃に枠番確定の出馬表が配信される
                      （公式仕様、docs/operations/DB_OPERATIONS_GUIDE.md 参照）。
                      dataspec単位で対応可否が異なる点は _WEEKLY_CAPABLE_DATASPECS 参照。
    ...
    def _resolve_option_and_from(ds: str, wm: str) -> tuple[int, str]:
        ...
        2026-07-03: JV-Data仕様書「蓄積系提供データ一覧」「データ提供タイミング表」
        （公式SDK JVDTLABSDK4902 付属）でこの実地検証結果の裏付けが取れた。
        option=2 対応 dataspec は TOKU/RACE/TCOV/RCOV/SNAP 系のみと明記されており、
        SLOP/WOOD の ret=-111 はバグではなく仕様通り。
        """
        if full_setup:
