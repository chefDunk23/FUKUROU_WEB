...
Usage:
    ...
    py -3.13-32 -m jvdl_client._downloader_32bit DATASPEC FROM_TIME OPTION --dry-run

    --dry-run: JVOpen のみ実行し "DRY_RUN:<rc>:<readcount>:<downloadcount>:<lastfile_ts>"
               を stdout に出力して即 JVClose する。ファイル書き込みなし。
...
def main() -> None:
    if len(sys.argv) not in (5,):
        print(
            f"Usage: {sys.argv[0]} DATASPEC FROM_TIME OPTION OUTPUT_FILE|--dry-run",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    output_file = None if dry_run else sys.argv[4]
    ...
    try:
        ...
        # ── dry-run: JVOpen 結果だけ返して即 JVClose ───────────────────────────
        if dry_run:
            # 64-bit 親が解析できるよう機械可読フォーマットで出力
            print(f"DRY_RUN:{ret}:{readcount}:{downloadcount}:{lastfile_ts}", flush=True)
            jv.JVClose()
            return
