...
BET-4（データ分割明文化）、BET-5（条件パターンの実験管理）、TR-0（調教データ抽出可否スパイク）です。
...
【TR-0に着手する場合の留意事項（人間からの指示・必ず守ること）】
- TR-0はTR-1（優先度抽出・順位付けロジック）の厳密なブロッカーである。TR-0未完了のままTR-1には着手しないこと。
- 目的は「データの有無」の確認ではない（\`training_slope\`/\`training_wood\`に既にデータが存在することはPLAN.md §3.5で確認済み）。
  \`time_Nf\`系フィールドが区間単独タイムか累積タイムか、\`lap_lX_lY\`系フィールドが想定区間と一致するか等、
  TR-1の優先度条件①〜⑦の正しい実装に必要な「フィールドの意味」を実データ・公式仕様書で確定すること。
- TR-0が完了したら、PROGRESS.mdに必ず単独の行で「TR-0: 完了」と記録してください。"
  fi
