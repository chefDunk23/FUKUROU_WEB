...
【次に着手する項目（人間からの指示、2026-06-26 START_LOOP=11更新）】今回はBET-5（条件パターンの実験管理・
複数戦略パターンの比較）に着手してください。BET-4はLoop10で完了済み（PROGRESS.md「BET-4: 完了」記録あり）
のため対象外です。TR-1も完了済みのため対象外です。
BET-6（本命選定ロジックのクリア数設計見直し）はPLAN.mdにバックログ項目として追加済みですが、今回も明示的に
スコープ外です。実装に着手しないでください（バックログのまま残す）。

【BET-5実装時の必須制約（人間からの指示・必ず守ること）】
BET-0〜BET-4・TR-0/TR-1の既存実装、特に \`tipster/combo_backtest.py\`、\`tipster/training_ranker.py\`、
\`tipster/engine.py\` の \`select_honmei()\`/\`select_aite()\` は、いずれも一切変更しないでください
（ロジック・出力結果を変えない。すべて既存のまま呼び出して再利用すること）。BET-5はこれらの上に
「複数の戦略パターンを切り替えて結果を比較できるようにする」実験管理レイヤーを追加するものであり、
既存の選定・バックテストロジック自体を新規実装し直す対象ではありません（PLAN.md §3 BET-5の
「提案する実装方針」参照）。
ただしPROGRESS.mdのEvaluatorフィードバックに既存項目（BET-0〜BET-4等）への未解決の指摘が残っている場合は、
...
- この規約はBET-3だけでなく、BET-5を含む今後の全ての回収率/スコア出力に適用する。"
  fi
