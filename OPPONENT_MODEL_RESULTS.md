...
C期間 N=3,449レースで v1フルモデルスコアと opponent_model スコアを比較。
...
**判断**: ランダム期待値（12頭立て想定: 約42%）より高いが、両モデルが強い馬を捉えるのは当然。上位1頭が74%独立しており追加価値あり。**v1スコア×opponent_modelスコアのアンサンブル**で使うのが最適。

### prev_rank系除外モデル（A期間OOF）

`prev_rank_norm`, `prev_rank`, `prev_margin` を除外して再学習した場合:

| モデル | カバー率 | 候補精度 | 複勝率 |
|-------|---------|---------|-------|
| 元モデル（21列） | 0.965 | 0.937 | 0.546 |
| 除外モデル（18列） | 0.926 | 0.876 | 0.406 |

**除外後のSHAP上位**: horse_age(0.141) > kinryo_vs_field(0.060) > distance_change(0.037) > opponent_next_avg_rank(0.009)

**判断**: 除外は採用しない。精度が-6.1ptと大幅に低下し、opponent_next系も0.009程度にとどまる。信号の弱さは欠損率26.4%と対戦相手ノイズが原因。**元モデル（21列）を採用**。
