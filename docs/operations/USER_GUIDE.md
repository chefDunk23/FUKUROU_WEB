...
> **2026-07時点で未使用**: `frontend/src`を検索してもこのタブに相当する画面が
> 現状存在せず、api_v1（動画生成API）を呼び出すコードも見つからない。
> `start.bat`の自動起動対象からも外した（本人確認済み・使用していない）。
> 以下は将来復活させる場合の参考として残す。

週末の予想ショート動画と月曜振り返り動画を生成する機能（過去の実装）。
