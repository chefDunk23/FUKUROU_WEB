...
| ケース | JV-Link同期 | DB同期 |
|--------|------------|-------|
| 木曜夕方（出走馬名表、枠番未確定） | ✅ 必須（`weekly=True`） | ✅ 必須 |
| 金曜・土曜（枠順確定後の定期更新） | ✅ 必須 | ✅ 必須 |
...
| データ種別 | 表示ラベル | 説明 |
|-----------|-----------|------|
| `RACE` | レース (RACE) | レース基本情報・出馬表・払戻。木曜16:30頃に枠番未確定版、金土12:00頃に枠番確定版が配信される（公式仕様、下記「JRA-VANの配信タイミング」参照） |
| `SLOP` | 調教坂路 (SLOP) | 坂路調教タイム（木・金更新） |
| `WOOD` | 調教ウッド (WOOD) | ウッドチップ調教タイム（木・金更新） |
| `DIFN` | 成績 (DIFN) | レース確定成績（月曜14:00頃更新） |
...
### JRA-VANの配信タイミング（公式仕様）

> 2026-07-03: JV-Data仕様書「蓄積系提供データ一覧」「データ提供タイミング表」で
> 確認済み。以前このガイドには「木曜=出馬投票確定」と記載していたが誤り。
> 木曜時点では**枠番・馬番が未確定**（`umaban=0`）の出走馬名表しか配信されない。

| タイミング | dataspec | 内容 |
|---|---|---|
| 日曜 夕刻 | `TOKU` | 特別登録馬・ハンデ発表前（翌週土日の特別レース登録馬。枠番なし） |
| 月曜 夕刻 | `TOKU` | 特別登録馬・ハンデ発表後（同上、負担重量が確定） |
| 木曜 15:00頃 | `RACE` | （例外）特定のG1レースがある場合のみ、そのレースの出走馬名表を先行提供。枠番・馬番がある場合もある |
| 木曜 16:30頃 | `RACE` | 出走馬名表発表（全場全レース、**枠番・馬番なし = umaban未確定**。初招待騎手・調教師の略称未設定） |
| 木曜 20:00頃 | `RACE` | 出走馬名表 更新版（初招待騎手・調教師の略称を設定したもの。枠番・馬番は引き続きなし） |
| **金曜・土曜 12:00頃** | `RACE` | **出馬表発表（枠番・馬番確定）** |
| 月曜 14:00頃 | `RACE` | 確定レース情報（前週末分） |

`JVOpen(option=2 / OPT_WEEKLY)` が対応する dataspec は `TOKU`/`RACE`/`TCOV`/`RCOV`/`SNAP` 系のみ。
`SLOP`/`WOOD`（調教データ）に `option=2` を渡すと `ret=-111`（エラー）になるのは仕様通りであり、
バグではない（2026-07-02 実地検証、`jvdl_client/sync_jvdata.py` の `_WEEKLY_CAPABLE_DATASPECS` 参照）。

`RACE` レコードの `data_kubun` フィールド（`races_v2.data_kubun` 列）で提供段階を判別できる:
`1`=出走馬名表(木曜), `2`=出馬表(金･土曜、枠番確定), `3`〜`6`=速報成績, `7`=成績(月曜),
`A`=地方競馬, `B`=海外国際レース, `9`=レース中止, `0`=該当レコード削除（JV-Data仕様書
「フォーマット」シート `RA` レコード 項番2 より）。

### 木曜夕方（出走馬名表取得 / 16:30 以降、枠番未確定）

```
1. JV-Link同期ボタン を実行 → worker.bat を起動
   → 出走馬名表（umaban=0、枠番・馬番未確定）が fukurou_jvdl に取り込まれる
   → 完了まで 5〜10 分待つ（ジョブ履歴が done になるまで）

2. DB同期ボタン を実行 → worker.bat を起動（既に閉じていれば）
   → fukurou_keiba_v2.races / race_entries に反映される（umaban=0のまま）

3. /db-status のデータ鮮度チェック（GET /api/v2/tipster/data-freshness）で
   「枠順未確定」警告が出ることを確認する
```

**この段階でできること:** 出走馬・騎手・血統情報の確認、対戦相手レベルの下調べ。
**この段階でできないこと:** 枠番・馬番に依存する予想生成（`field_size` 補完頼みの暫定値になる）。

### 金曜・土曜（枠順確定後 / 12:00 以降）

```
1. JV-Link同期ボタン を実行 → worker.bat を起動
   → 週末レース出馬表（枠番・馬番確定）・調教データが fukurou_jvdl に取り込まれる
   → 完了まで 5〜10 分待つ（ジョブ履歴が done になるまで）
